...
CALL PopulateDateDimension('2005/05/01', '2006/02/15');
...
ADD COlUMN film_key int;

UPDATE fact_rental as fr
set fr.film_key = (
 select dim_film.film_key
 from sakila.inventory 
 left join dim_film on  sakila.inventory.film_id = dim_film.film_key
 where sakila.inventory.inventory_id = fr.inventory_id);

UPDATE sakila_2.fact_rental AS fr
JOIN sakila_2.dim_date AS dd
...
UPDATE sakila_2.fact_rental AS fr
JOIN sakila_2.dim_date AS dd
ON DATE(fr.last_update) = dd.full_date
SET fr.last_update_key = dd.date_key;
